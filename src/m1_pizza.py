...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(200, 100),  # All numbers are approximate.
                rg.Point(100, 200),
                rg.Point(0, 100),
                rg.Point(100, 0)]
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  40 blue on a yellow circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.fill_color = 'yellow'
    draw_points_on_circle(window, circle, 40, 'blue')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 100, 'yellow', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    points_for_circle=(generate_points_on_circle(circle, number_of_slices * 2))
    for k in range(number_of_slices):
        new_line = rg.Line(points_for_circle[k], points_for_circle[k + number_of_slices])
...
def run_test_polygon():
    ...
    title = 'POLYGON test 3:  6 segments with thick black lines;  12 with thick gold lines.'
    window = rg.RoseWindow(650, 500, title)

    circle = rg.Circle(rg.Point(200, 200), 90)
    circle.outline_thickness = 6
    polygon(window, circle, 6, "black", 20)
    window.continue_on_mouse_click()

    circle = rg.Circle(rg.Point(450, 300), 250)
    circle.outline_thickness = 6
    polygon(window, circle, 12, "gold", 10)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    points_for_circle = generate_points_on_circle(circle, number_of_segments )
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  10 orange lines on blue circle, hops = 4.'
    window = rg.RoseWindow(480, 350, title)
